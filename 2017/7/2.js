function fn(input) {
  ...
  const [tree, byChildren] = lines
    .map((line, i) => line.match(/(\w+) \((\d+)\)($|( -> (.*)))/))
    .reduce(
      ([tree, children], match) => {
        if (!match) throw new Error(`Parse error on: ${tree}`);
        const name = match[1];
        const node = {
          weight: parseFloat(match[2]),
          children: match[5] && match[5].split(", ")
        };
        tree[name] = node;
        children[name] = children[name] || undefined;
        node.children &&
  ...
  // Fill in full weights
  for (const [k, node] of Object.entries(tree)) {
    if (!node.children) continue;
    for (const c of node.children) {
      calculateWeightFor(c, tree);
    }
  }

  const [parent, _] = Object.entries(byChildren).find(
    ([child, parent]) => parent == null
  );

  return getCorrectWeight(tree, parent);
}

function getCorrectWeight(tree, name) {
  const node = tree[name];
  if (!node.children) return null;

  const firstWeight = tree[node.children[0]].fullWeight;
  const equalToFirst = node.children.filter(
    (child, i, arr) => firstWeight === tree[child].fullWeight
  );

  // Balanced, all children the same
  if (equalToFirst.length === node.children.length) return null;

  // First is unbalanced
  let idealWeight = tree[node.children[1]].fullWeight;
  let unbalanced = 0;
  // Or, someone else is, find it and check
  if (equalToFirst.length > 1) {
    idealWeight = firstWeight;
    unbalanced = node.children.findIndex(
      n => tree[n].fullWeight !== firstWeight
    );
  }

  // Get the correct weight from nested children if possible
  let correctWeight = getCorrectWeight(tree, node.children[unbalanced]);

  if (!correctWeight) {
    // Children were balanced, so this node is the offender
    const blackSheep = tree[node.children[unbalanced]];
    return blackSheep.weight + (idealWeight - blackSheep.fullWeight);
  }

  return correctWeight;
}

function calculateWeightFor(nodeName, tree) {
  const node = tree[nodeName];
  if (!node.fullWeight) {
    if (!node.children) node.fullWeight = node.weight;
    else
      node.fullWeight =
        node.weight +
        node.children.reduce((s, n) => s + calculateWeightFor(n, tree), 0);
  }
  return node.fullWeight;
}
